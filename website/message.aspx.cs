...
namespace website
{
    public partial class message : System.Web.UI.Page
    {
        ...
        public String[] un = new string[10];//用户名
        public String[] ms = new string[10];//留言
        ...
        string str = "Data Source = localhost; Initial Catalog = blog; Persist Security Info = True; User ID = sa; Password = 123456";
            ...
            myReader.Close();
            String str1 = "select * from message";
            SqlCommand sqlCmd1 = new SqlCommand(str1, sqlCon);
            SqlDataReader myReader1 = sqlCmd1.ExecuteReader();
            while (myReader1.Read())
            {
                un[i] = myReader1.GetValue(0).ToString();
                ms[i] = myReader1.GetValue(1).ToString();
                time[i] = myReader1.GetValue(2).ToString();
                i = i + 1;
            }
        ...
        protected void Button6_Click(object sender, EventArgs e)
        {
            ...
            string str = "Data Source = localhost; Initial Catalog = blog; Persist Security Info = True; User ID = sa; Password = 123456";
            SqlConnection sqlCon = new SqlConnection(str);
            str = "insert into message (username,message,time) values ('" + Session["username"] + "','" + Textares1.Value + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "')";
            SqlCommand sqlCmd = new SqlCommand(str, sqlCon);
            sqlCon.Open();
            ...
            if (insert != 0)
            {
                Response.Write("<script>location.href='message.aspx';</script>");
            }
            }
            else
            {
